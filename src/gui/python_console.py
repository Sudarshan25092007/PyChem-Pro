...
class PythonConsole(QWidget):
    ...
    def _morgan_fp(self, radius=2, n_bits=2048):
        ...
        from ..core.fingerprints import get_morgan_fingerprint
        mol = self._namespace.get('mol')
        if not mol:
            self._append_text("No molecule loaded.", COLORS['warning'])
            return None
    ...
    def _topological_fp(self, min_path=1, max_path=7, n_bits=2048):
        ...
        from ..core.fingerprints import get_topological_fingerprint
        mol = self._namespace.get('mol')
        if not mol:
            self._append_text("No molecule loaded.", COLORS['warning'])
            return None
    ...
    def _maccs_keys(self):
        ...
        from ..core.fingerprints import get_maccs_keys
        mol = self._namespace.get('mol')
        if not mol:
            self._append_text("No molecule loaded.", COLORS['warning'])
            return None
    ...
    def _similarity(self, fp1, fp2):
        ...
        from ..core.fingerprints import tanimoto_similarity
