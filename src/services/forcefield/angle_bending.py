...
class AngleBendingCalculator:
        ...
        for atom in mol.atoms:
            ...
            for a in range(len(neighbors)):
                for b in range(a + 1, len(neighbors)):
                    ...
                    theta0_deg, ka = get_angle_params(sym_i, atom.symbol, sym_k, hyb)
    ...
    def gradient(self, coords, angles):
        ...
        for i, j, k, theta0, ka in angles:
            theta = self._compute_angle(coords[i], coords[j], coords[k])
            dt = theta - theta0
            dE_dtheta = 0.021914 * ka * (2.0 * dt * (1 + self.CB * dt) + dt * dt * self.CB)
            g_i, g_j, g_k = self._angle_gradient(coords[i], coords[j], coords[k])
